...
def main() -> int:
    ...
    render_hub(env, "dish",
               ...
               f"Every dish on WorldDishAtlas, A to Z. Canonical recipes, history, and where to eat each one.",
               sorted_dishes,
    ...
    render_index(env, "cuisine",
                 ...
                 "Browse WorldDishAtlas by cuisine. Italian, French, Japanese, Thai, Mexican, and more.",
    ...
    for cuisine, dlist in by_cuisine.items():
        ...
        render_hub(env, f"cuisine/{slug}",
                   ...
                   f"Every {cuisine} dish on WorldDishAtlas. Canonical recipes, regional histories, and where to eat each one.",
    ...
    render_index(env, "ingredient",
                 ...
                 "Browse WorldDishAtlas by ingredient. Find every dish that uses guanciale, fish sauce, saffron, and more.",
    ...
    for ing, dlist in by_ingredient.items():
        ...
        render_hub(env, f"ingredient/{ing}",
                   ...
                   f"Every dish on WorldDishAtlas that uses {name.lower()} as a core ingredient.",
                   sorted(dlist, key=lambda d: d["name"]),
                   [{"name": "Home", "url": f"{BASE}/"},
    ...
    for cat, dlist in by_category.items():
        ...
        render_hub(env, f"category/{slug}",
                   ...
                   f"Every {cat} dish covered on WorldDishAtlas. Canonical recipes and where to eat each one.",
                   sorted(dlist, key=lambda d: d["name"]),
                   [{"name": "Home", "url": f"{BASE}/"},
                    {"name": cat.title()}])
